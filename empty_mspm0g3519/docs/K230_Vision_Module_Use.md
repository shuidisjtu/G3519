...
版本：V1.5。**链路已实测打通**（2026-07-17：模拟帧；2026-07-19：色块追踪 LCD 画框，§4.1），G3519 侧驱动/协议解析/菜单已实装（§4）。⚠️ 遗留一个硬件待解决问题：J11/J4 的 1 脚 5V 供电能力不足（§7）。
...
| 色块追踪 | **开发中**：原厂颜色识别帧 → LCD 绿色空心矩形框标出色块位置 + 底部状态栏（§4.1） |
...
### 4.1 色块追踪 — LCD 画框（开发中，2026-07-19）
