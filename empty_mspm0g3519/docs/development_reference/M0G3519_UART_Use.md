...
UART3（TJC 串口屏通道，2026-07-27 双向通信验证通过）：

| 项目 | 配置 |
|------|------|
| UART 实例 | UART3 (PD1, 主 UART)，SysConfig 实例名 `UART_TJC` |
| 时钟源 | BUSCLK = 80 MHz |
| 波特率 | 115200, 8N1, 无流控 |
| 引脚 | PC6 (TX) / PC7 (RX)，J4 排针 |
| 供电注意 | J4 Pin1 (5V) 带载不足，TJC 屏需从其他 5V 接口独立供电 |
...
3. 烧录并运行程序，开机后可通过命令协议验证：发送 `VER?\r\n`，应收到 `OK,G3519-Signal,v1.0\r\n`
4. UART0 已加 10ms TX 超时保护，脱机（不接 DAPLink）时启动正常，printf 静默失败不卡死
