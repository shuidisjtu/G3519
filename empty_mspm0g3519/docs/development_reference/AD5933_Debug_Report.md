...
**文件:** `iar/empty_mspm0g3519.c:178-358`（`action_ad5933_test()`）
...
2. 回读 CTRL_H/CTRL_L 寄存器，显示在 LCD Row 1（`C: B1 08`）
3. LCD 提示接入 18kΩ 校准电阻，按 S2 确认
...
Row 1: C: B1 08            ← CTRL 寄存器回读 (WHITE)
...
| API | 说明 |
|-----|------|
| `tsp_ad5933_init()` | 复位 + 外部时钟 + 待机（CTRL_H=0xB1）|
...
| P2 | CTRL_H 回读 | LCD Row 1 | `B1 08` = STANDBY + PGA_X1 + 2.0Vpp |
| P3 | 1kHz 激励输出 | AD2 Scope CH1 @ J15-5 | 1.000kHz / ~2Vpp 正弦波，稳压 |
| P4 | DUT 电流 | 万用表 @ DUT 两端 | 18kΩ 约 1Vp / 56μA |
...
|-----|------|---------|---------|
| 18kΩ | 111μApp | 2.22Vpp | ~16000（满量程）✅ |

**本板实际**（U6A ×100Ω → R36=20k → VIN）：

| DUT | I_DUT | U6A 输出 | VIN 注入电流 | TIA 输出 | DFT 估算 |
|-----|-------|---------|------------|---------|---------|
| 18kΩ | 111μA | 11.1mVpp | 0.56μApp | **11.1mVpp** | ~64 counts ❌ |
...
- 18kΩ 时 VIN 注入电流 ≈ 111μApp，TIA 输出 ≈ 2.2Vpp（满量程）
- 固件标定/测量流程**无需任何修改**
