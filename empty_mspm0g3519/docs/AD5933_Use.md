...
进入后自动执行初始化并在单页 LCD 上显示全部信息：

```
┌──────────────────────┐
│AD5933 Impedance      │ ← Row 0, YELLOW/BLUE
│C: B0 08              │ ← Row 1, WHITE (CTRL_H/L 回读)
│T: 32.1C              │ ← Row 2, WHITE (每 1s 自动刷新)
│Freq: 1000 Hz         │ ← Row 3, WHITE
│Real:  xxxxx          │ ← Row 4, WHITE (每 50ms 刷新)
│Imag:  xxxxx          │ ← Row 5, WHITE (每 50ms 刷新)
│Mag :  xxxxx          │ ← Row 6, GREEN (每 50ms 刷新)
...
2. 回读 CTRL_H/CTRL_L 并显示（验证 P2 修复：`AD5933_CTRL_STANDBY` 16-bit 宏正确写入）
3. `tsp_ad5933_read_temperature()` — 测量温度并显示（验证 P6 修复：14-bit 掩码 + 符号扩展）
4. `set_sweep(1000, 0, 0, 100, X1)` — 配置单频点扫频
5. `start_sweep()` — 启动 DDS + DFT（验证 P3 修复：频率字公式 `MCLK/4` 分频）
6. 进入 live loop：Real/Imag/Mag 每 50ms 刷新，温度每 1s 自动刷新

**按键**：
- `PUSH`：退出并返回主菜单（退出时自动发送掉电命令）
