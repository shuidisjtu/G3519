...
static void action_dds_test(void)
{
	...
	/* Start DDS with encoder ISR for reliable quadrature decode */
...

	{
		...
		while (1) {
			...
			/* --- Keys + encoder every ~10ms --- */
			if (tick >= 10) {
				tick = 0;

				/* Read encoder delta from ISR-driven counter.
				 * ISR yields 2 counts/detent (both PHA0 edges).
				 * Step = 25 Hz/count → 50 Hz per detent. */
				int32_t cur_count = tsp_encoder_get_count();
				int32_t enc_acc   = cur_count - last_count;
				 ...
				 * ISR yields 2 counts/detent (both PHA0 edges).
				 * 25 Hz/count → 50 Hz per detent. */
				if (enc_acc != 0) {
					int32_t new_freq = (int32_t)freq + enc_acc * 25;
