...
>
> 注：§10.3 问题数据表中的 "220Ω（标定）" 是 2026-07-24 调试报告测试当时实际使用的标定电阻；当前代码校准电阻为 300KΩ。更换 R36 后复测时应以当前固件流程（300KΩ 标定）为准。
...
| `signal/iar/empty_mspm0g3519.c` | `action_ad5933_test()` 菜单回调（仅 signal 工程） |
...
| P6 | 温度读取（14-bit 掩码 + 符号扩展） | LCD | `T: 32.1°C`，手指按压后上升 ✅（显示格式 `T: 2x.xC`，当前测试界面不显示温度） |
| I2C | 总线通信 | AD2 Logic | `0x0D WR`，各字节 ACK ✅（AD2 Logic 解码显示的是原始地址字节 `0x1A`，即 7-bit 地址 `0x0D` 左移一位，属正常） |
| P3 | 频率字公式（MCLK/4 分频） | AD2 Scope CH1 | 1.000 kHz / ~2Vpp 正弦波 ✅ |

> **通过标准速查**（2026-08-11 恢复自 AD5933_Examine.md §四）：

| 步骤 | 看哪里 | 期望显示 | 验证内容 |
|------|--------|---------|---------|
| 第一步 | AD2 Logic | I2C 解码为 7-bit 写地址 `0x0D`（原始地址字节 `0x1A`）→ **ACK** | J19/上拉/供电 |
| 第二步 | LCD Row 1 | `C: B3 08` | P2：CTRL_H 写入正确 |
| 第三步 | LCD Row 2 | `T: 2x.xC`（当前测试界面不显示温度） | P6：温度掩码 + 符号扩展 |
...
> 注：此 10kΩ 为链路验证建议值，与 §2 标定的 300KΩ、§10.3 记录用的 220Ω 用途不同（链路验证 / 标定 / 问题数据复现），三者不冲突。
...
> 测试条件：标定电阻 220Ω（2026-07-24 当时实际使用，流程见 §10.6 软件实现细节）；当前代码标定电阻为 300KΩ。更换 R36 后复测应使用当前固件流程（300KΩ 标定）。
...
### 10.6 软件实现细节（2026-08-11 恢复自原调试报告"二、软件实现"节）

**文件：** `signal/iar/empty_mspm0g3519.c`（`action_ad5933_test()`，菜单表）
**依赖：** `common/drivers/tsp_ad5933.c/.h`（或 NUEDC2025，以工程实际为准）
...
3. LCD 提示接入标定电阻，按 S2 确认（2026-07-24 测试时用 220Ω；当前代码 `cal_resistance = 300000.0f` 对应 300KΩ）
...
8. **GainFactor = 1 / (R_cal × Mag_cal)**（2026-07-24 时为 1 / (220 × Mag_cal)）
...
> 与 §2 自动流程对照：§2 为当前固件界面描述（4 次 REPEAT_FREQ 平均标定、`PUSH` 任意阶段退出），本节为 2026-07-24 调试时的完整轮询/超时参数记录，二者流程一致，本节补充了超时与节奏细节。
