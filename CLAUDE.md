...
└── NUEDC2025/                         ← 应用层驱动
    ...
    └── tsp_dds.h/.c                   ← AD9833 DDS 波形发生器 [已封存：PC2/PC3 与 CCD 共用]
```
...
| DDS [封存] | PC2(SCLK), PC3(SDATA), PC24(FSYNC) | `DDS_SCLK/SDATA/FSYNC` ⚠️ PC2/PC3 与 CCD 共用 |
...
| MPU6050 (I2C0) | PB21(SCL), PB22(SDA), PC8(INT) | 六轴 IMU，2.2kΩ 上拉，无驱动代码 |
...
tsp_key_init();                        // 按键
tsp_menu_init(title, items, count);    // 菜单（4 项：K230/CCD/AD5933/Motor Test）
...
// LCD 波形绘制见 action_ccd_test(): 128px × 80px 区域，erasure-based 增量更新
// AD2 调试: 探头接 SI(PC9) → 触发帧起始; CLK(PB20) → 像素时钟; AO(PB18) → 模拟输出
...
// ===== AD9833 DDS Waveform Generator [已封存: PC2/PC3 与 CCD 共用] =====
// 如需启用: 1) 取消注释 #include "tsp_dds.h"  2) 恢复菜单项  3) 确保 CCD 未初始化
...
// ===== DRV8874 DC Motor Driver（TIMA0 手动初始化, 不在SysConfig中, 详见 docs/DRV8874_Motor_Use.md）=====
